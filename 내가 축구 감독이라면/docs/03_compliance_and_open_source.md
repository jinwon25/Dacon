...
| 요구사항 | 현재 상태 | 판단 | 조치 |
|---|---|---|---|
| 실제 월드컵 데이터 활용 | StatsBomb 경기 ID 3857262·3869321의 실제 이벤트, 공간 좌표, 패스 연결과 이벤트 시퀀스 사용 | 적합 | 화면과 결과 설명에 관측값·모델값을 분리 표시 |
...
다음 명령은 두 시나리오의 팀별 관측값과 인플레이 점유 추정을 원본 이벤트 JSON에서 다시 계산합니다.
