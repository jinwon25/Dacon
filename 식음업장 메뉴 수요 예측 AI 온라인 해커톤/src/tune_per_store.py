...
GLOBAL = dict(wd=0.4, m28=0.6, nz=0.0, c=1.0)  # 현재 전역 최고
COMP = ["wd", "m28", "nz"]  # 블렌드 성분 (요일평균 / 28일평균 / 비-0평균)


def simplex(step=0.25):
    ks = int(round(1 / step))
    return [(i * step, j * step, (ks - i - j) * step)
            for i in range(ks + 1) for j in range(ks + 1 - i)]
...
def search(df, scales=None):
    ...
    for store, g in df.groupby("store"):
        ...
        bcfg, bsc = None, 9.0
        for a, b, c2 in simp:
            base = a * comps["wd"] + b * comps["m28"] + c2 * comps["nz"]
            for s in scales:
                sc = weighted_smape(true, np.clip(s * base, 0, None))
                if sc < bsc:
                    bsc, bcfg = sc, dict(wd=a, m28=b, nz=c2, c=float(s))
        best[store] = (bcfg, bsc)
    return best
...
def main():
    ...
    for store, (cfg, sc) in best.items():
        ...
        rows.append(dict(store=store, high_w=store in ("담하", "미라시아"),
                         wd=cfg["wd"], m28=cfg["m28"], nz=cfg["nz"], c=cfg["c"],
                         before=round(before, 4), after=round(sc, 4),
