"""v111_boundary_on_v109.py - Boundary MLP on v109 MDN-WTA wmean.

v109 wmean base OOF 0.6538 (낮음). v90 mirror 0.6643 → v94 boundary 0.6738
패턴 적용 시 v109 + boundary ~ 0.663+ 기대.

핵심: v109 paradigm 진정성 (MDN multi-modal) + boundary refinement.
v91/v94와 동일 BoundaryMLP, base만 v109 absolute pred로 변경.

cache: v111_{cap_cm}_state.npz
"""
...
def main():
    ...
    parser.add_argument("--base-pool", default="v109_K4_pool.npz",
                        help="absolute-pred pool cache (keys: oof_wm/test_wm)")
    parser.add_argument("--base-key", default="wm", choices=["wm", "am"],
                        help="which v109 variant to refine")
    parser.add_argument("--out-tag", default="v111", help="prefix for output")
    args = parser.parse_args()

    torch.manual_seed(0); np.random.seed(0)
    torch.set_num_threads(os.cpu_count() or 4)
    cap_lbl = f"{int(round(args.cap_cm*10)):02d}"
    tag = f"{args.out_tag}_{args.base_key}_cap{cap_lbl}"
    print("=" * 60)
    print(f"v111 boundary MLP on v109 {args.base_key} (cap {args.cap_cm}cm)")
    print(f"  base = {args.base_pool} / oof_{args.base_key}")
    print(f"  tag = {tag}")
    print("=" * 60)
    ...
    pool = np.load(CACHE_DIR / args.base_pool)
    oof_base = pool[f"oof_{args.base_key}"].astype(np.float64)
    test_base = pool[f"test_{args.base_key}"].astype(np.float64)
    rh_base = float((np.linalg.norm(oof_base - y_train, axis=-1) <= 0.01).mean())
    print(f"v109 {args.base_key} base OOF: {rh_base:.4f}")
    ...
    entry = {"version": tag, "ts": _dt.datetime.now().isoformat(timespec="seconds"),
             "approach": f"boundary MLP cap={args.cap_cm}cm on v109 {args.base_key}",
             "rh_base": float(rh_base), "rh": float(rh),
             "delta_base": float(rh - rh_base)}
