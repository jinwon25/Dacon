"""v105_bilstm_mirror.py — Bidirectional LSTM + y-mirror aug + TTA.

v77 BiGRU framework에서 GRU→LSTM 교체. paradigm 다양성용.
GRU는 update/reset gate, LSTM은 input/forget/output gate + cell state.
짧은 11 step에서 LSTM은 long-term memory가 약간 다른 inductive bias 가짐.

설계:
  - v77/v90 framework 그대로 (Kalman residual + multi-aux + mirror TTA)
  - GRU → LSTM 1-layer 양방향
  - hidden=64 동일
  - 5-fold × 2-seed × 150ep
  - setup A/B 선택 가능

기대:
  - v90 BiGRU mirror OOF 0.6643 → v105 BiLSTM mirror ?
  - architecture diversity 확보 (Trans + GRU + LSTM)
  - boundary MLP 적용 시 v94 (0.6738)와 ensemble lift 가능

별도 cache: v105{suffix}_state.npz
"""
...
def mirror_seq(seq):
    out = seq.copy(); out[...,1]*=-1; out[...,4]*=-1; out[...,7]*=-1; return out
def mirror_target(t):
...
class BiLSTMMultiAux(nn.Module):
    """양방향 LSTM + main/F/W aux heads (v77 BiGRU 구조 LSTM 변형)."""
    def __init__(self, n_channels=9, scal_dim=40, hidden=64, fc=128, p=0.2,
                 aux_dims=(3, 3), main_scale_cm=2.0):
        super().__init__()
        self.lstm = nn.LSTM(n_channels, hidden, num_layers=1, batch_first=True, bidirectional=True)
...
def run_kfold(target_main, target_F, target_W,
    ...
    for fi, (tr, va) in enumerate(fold_iter):
        ...
        print(f"  ★ fold {fi+1}/{n_folds}: R-Hit={rh_fold:.4f}  ({(time.time()-t0)/60:.1f}m)  "
              f"[mirror={mirror_on}, BiLSTM]", flush=True)
    ...
    print(f"  OOF R-Hit: {oof_rhit:.4f}  소요 {(time.time()-t0)/60:.1f}m  [BiLSTM+mirror]")
...
def main():
    ...
    parser.add_argument("--tag", default="v105")
    ...
    print("=" * 60)
    print(f"v105 BiLSTM + y-mirror={args.mirror} + TTA  setup={args.setup}")
    ...
    print(f"\n[v105] OOF cal: {rh_cal:.4f}  (v90 BiGRU mirror: 0.6643)")
    ...
    entry = {"version": args.tag, "ts": _dt.datetime.now().isoformat(timespec="seconds"),
             "approach": f"BiLSTM + y-mirror={args.mirror} + TTA + multi-aux",
