...
CACHE = ROOT / "cache"; REPORTS = ROOT / "reports"; FINAL = ROOT / "final_candidates"
DT = 0.040

# data
xc = np.load(CACHE / "xtrain_xtest.npz")
X_train = xc["X_train"]; X_test = xc["X_test"]
labels = pd.read_csv(ROOT / "open" / "train_labels.csv")
...
if best_lift >= GATE:
    ...
    sample = pd.read_csv(ROOT / "open" / "sample_submission.csv")
