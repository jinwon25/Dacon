"""v55_soft_stacker_v4.py — 13-model SoftStacker (v53 + v54 Trans cap variants).

v53 (11m, with v52_05/v52_15 GRU cap variants) OOF 0.6746.
v54 cap variants on Trans base (v44 framework) — paradigm orthogonal.
13 models: 9 + v52_05/v52_15 (GRU boundary cap) + v54_05/v54_15 (Trans boundary cap).
"""
...
class SoftStacker(nn.Module):
    def __init__(self, in_dim, n_models, hidden=128, p=0.4, temp=1.5):
        super().__init__()
...
def main():
    ...
    print("=" * 60)
    print(f"v55 SoftStacker v4 — 13 models")
    print("=" * 60)
    ...
    # v54 Trans boundary cap variants (stored with v44-script key 'oof_v44')
    st54_05 = np.load(CACHE_DIR / "v54_cap0p5_state.npz")
    v54_05_oof, v54_05_te = st54_05["oof_v44"].astype(np.float64), st54_05["test_v44"].astype(np.float64)
    st54_15 = np.load(CACHE_DIR / "v54_cap1p5_state.npz")
    v54_15_oof, v54_15_te = st54_15["oof_v44"].astype(np.float64), st54_15["test_v44"].astype(np.float64)

    models_train = [v30A_oof, v30B_oof, v35_oof, v41A_oof, v41B_oof, v44_oof, gate_oof,
                    v39_oof, v32_oof, v52_05_oof, v52_15_oof, v54_05_oof, v54_15_oof]
    models_test  = [v30A_te,  v30B_te,  v35_te,  v41A_te,  v41B_te,  v44_te,  gate_te,
                    v39_te,  v32_te,  v52_05_te, v52_15_te, v54_05_te, v54_15_te]
    names = ["v30A", "v30B", "v35", "v41A", "v41B", "v44", "gate", "v39", "v32",
             "v52_05", "v52_15", "v54_05", "v54_15"]
    ...
    print(f"\n  Oracle (any of 13): {oracle:.4f}  (vs 11m: 0.7228)")
    ...
    oof_v55 = np.mean(oofs, axis=0)
    test_v55 = np.mean(tests, axis=0)
    w_mean = np.mean(all_w, axis=0)
    rh_v55 = rhit(oof_v55, y_train)

    print(f"\n=== v55 SoftStacker v4 (13m) 결과 ===")
    print(f"  v48 (9m)      : 0.6734")
    print(f"  v53 (11m)     : 0.6746")
    print(f"  v55 (13m)     : {rh_v55:.4f}  (Δ vs v53: {rh_v55 - 0.6746:+.4f})")
    print(f"  weight avg:")
    ...
    try:
        ...
        best_3 = (1.0, 0.0, 0.0); best_3r = rh_v55
        for a in np.linspace(0, 1, 11):
            for b in np.linspace(0, 1-a, 11):
                c = 1 - a - b
                ens = a*oof_v55 + b*oof_v53 + c*v35_oof
                r = rhit(ens, y_train)
                if r > best_3r:
                    best_3r, best_3 = r, (a, b, c)
        print(f"\n  3-way v55/v53/v35: {best_3} OOF={best_3r:.4f}")

        best_4 = (1.0, 0.0, 0.0, 0.0); best_4r = rh_v55
        for a in np.linspace(0, 1, 6):
            for b in np.linspace(0, 1-a, 6):
                for c in np.linspace(0, 1-a-b, 6):
                    d = 1 - a - b - c
                    if d < 0: continue
                    ens = a*oof_v55 + b*oof_v53 + c*v35_oof + d*oof_v48
                    r = rhit(ens, y_train)
                    if r > best_4r:
                        best_4r, best_4 = r, (a, b, c, d)
        print(f"  4-way v55/v53/v35/v48: {best_4} OOF={best_4r:.4f}")
    except Exception as e:
        print(f"  hybrid cache err: {e}")
        best_3, best_3r = (1.0, 0.0, 0.0), rh_v55
        best_4, best_4r = (1.0, 0.0, 0.0, 0.0), rh_v55
        oof_v53 = test_v53 = oof_v48 = test_v48 = None

    np.savez(CACHE_DIR / "v55_state.npz",
             oof_v55=oof_v55, test_v55=test_v55, rh_v55=rh_v55,
             weights_oof_mean=w_mean)

    out_csv = DATA_DIR / "submission_v55_cpu.csv"
    pd.DataFrame({"id": sub["id"], "x": test_v55[:,0], "y": test_v55[:,1], "z": test_v55[:,2]}
                 ).to_csv(out_csv, index=False)
    print(f"\n  [submission] {out_csv}")

    if best_3r > rh_v55 and oof_v53 is not None:
        a, b, c = best_3
        hyb = a*test_v55 + b*test_v53 + c*v35_te
        hyb_csv = DATA_DIR / f"submission_v55_3way_v55x{a:.2f}_v53x{b:.2f}_v35x{c:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb[:,0], "y": hyb[:,1], "z": hyb[:,2]}
                     ).to_csv(hyb_csv, index=False)
        print(f"  [3way] {hyb_csv.name}")

    if best_4r > rh_v55 and oof_v53 is not None and oof_v48 is not None:
        a, b, c, d = best_4
        hyb = a*test_v55 + b*test_v53 + c*v35_te + d*test_v48
        hyb_csv = DATA_DIR / f"submission_v55_4way_v55x{a:.2f}_v53x{b:.2f}_v35x{c:.2f}_v48x{d:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb[:,0], "y": hyb[:,1], "z": hyb[:,2]}
                     ).to_csv(hyb_csv, index=False)
        print(f"  [4way] {hyb_csv.name}")

    entry = {
        "version": "v55_soft_stacker_v4",
        "ts": _dt.datetime.now().isoformat(timespec="seconds"),
        "approach": "13-model SoftStacker (v52 GRU + v54 Trans cap variants)",
        "n_models": len(names),
        "model_names": names,
        "oracle_any13": float(oracle),
        "v55_oof": float(rh_v55),
        "delta_vs_v53": float(rh_v55 - 0.6746),
        "weight_avg": {n: float(wi) for n, wi in zip(names, w_mean.mean(axis=0))},
        "3way_v55_v53_v35": list(best_3),
        "3way_oof": float(best_3r),
        "4way_v55_v53_v35_v48": list(best_4),
    ...
    print(f"\n  [run_log] appended v55_soft_stacker_v4")
