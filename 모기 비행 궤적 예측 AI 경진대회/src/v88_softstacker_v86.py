"""v88_softstacker_v78.py — 14-model SoftStacker (v75 + v78).

v79: 4-way (v48_9m / v46_7m / v35 / v78) = (0.70/0.09/0.19/0.02) → OOF 0.6749 (+0.0001)
v78 paradigm이 처음으로 미세 lift. SoftStacker에서 더 큰 weight 가능?

14-model: v48 9 + v73s + v73h + v65h + v62 + v78
"""
...
def main():
    ...
    st86 = np.load(CACHE_DIR / "v86_state.npz")

    models = [v30A_o, v30B_o, v35o, v41A_o, v41B_o, v44o, gate_o, v39o, v32o,
              st73["oof_soft"].astype(np.float64), st73["oof_hard"].astype(np.float64),
              st65["oof_hard"].astype(np.float64), v62o,
              st78["oof_v78"].astype(np.float64),
              st86["oof_v86"].astype(np.float64)]
    tests  = [v30A_t, v30B_t, v35t, v41A_t, v41B_t, v44t, gate_t, v39t, v32t,
              st73["test_soft"].astype(np.float64), st73["test_hard"].astype(np.float64),
              st65["test_hard"].astype(np.float64), v62t,
              st78["test_v78"].astype(np.float64),
              st86["test_v86"].astype(np.float64)]
    names = ["v30A","v30B","v35","v41A","v41B","v44","gate","v39","v32",
             "v73s","v73h","v65h","v62","v78","v86"]
    K = len(names)
    print(f"v88 SoftStacker — {K} models")
    ...
    print(f"  Oracle ({K}-m): {hits.any(axis=0).mean():.4f}")
    ...
    base_o = 0.70 * np.load(CACHE_DIR / "v48_state.npz")["oof_v48"] \
           + 0.12 * np.load(CACHE_DIR / "v46_state.npz")["oof_v46"] + 0.18 * v35o
    base_t = 0.70 * np.load(CACHE_DIR / "v48_state.npz")["test_v48"] \
           + 0.12 * np.load(CACHE_DIR / "v46_state.npz")["test_v46"] + 0.18 * v35t
    rh_base = rhit(base_o, y_train)
    print(f"\n=== v88 결과 ===")
    print(f"  base : {rh_base:.4f}")
    print(f"  v88 ({K}m): {rh_v:.4f}  (Δ vs base: {rh_v - rh_base:+.4f})")
    for n, wi in zip(names, w_mean.mean(axis=0)):
        flag = " ★" if n in ("v73s","v73h","v65h","v62","v78") else ""
        print(f"    {n:6s}: {wi:.3f}{flag}")
    ...
    print(f"\n  hybrid base×w + v88: best w={best_w:.2f} → OOF {best_r:.4f}  Δ {best_r - rh_base:+.4f}")

    out = DATA_DIR / "submission_v88_15m.csv"
    ...
    if best_w < 1.0 and best_r > rh_base:
        ...
        h = DATA_DIR / f"submission_v88_hyb_basex{best_w:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb_t[:,0], "y": hyb_t[:,1], "z": hyb_t[:,2]}).to_csv(h, index=False)
        print(f"  [hybrid] OOF {best_r:.4f} → {h.name}")

    np.savez(CACHE_DIR / "v88_state.npz", oof=oof, test=test, rh=rh_v, w_mean=w_mean)
    entry = {"version": "v88_softstacker_v78", "ts": _dt.datetime.now().isoformat(timespec="seconds"),
