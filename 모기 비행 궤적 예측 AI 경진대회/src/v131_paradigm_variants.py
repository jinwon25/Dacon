...
def run_kfold(X_train, X_test, y_train, R_train, R_test, X_scal_tr, X_scal_te,
    ...
    def make_model():
        ...
        if encoder in ("transformer", "lru", "tcn", "itransformer", "flow", "sonode"):
            from encoders import ENCODER_REGISTRY   # lazy: gru/mlp 경로 무영향
            return ENCODER_REGISTRY[encoder](seq_channels=C, scal_dim=scal_dim,
                               latent_dim=cfg.get("latent_dim", 64), hidden=cfg.get("hidden", 64),
                               n_steps=cfg.get("n_steps", 1))
...
def main():
    ...
    ap.add_argument("--encoder", default="mlp", choices=["mlp", "gru", "transformer", "lru", "tcn", "itransformer", "flow", "sonode"])
