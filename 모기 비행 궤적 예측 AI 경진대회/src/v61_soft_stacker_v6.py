...
def main():
    ...
    print("=" * 60)
    print(f"v61 SoftStacker v6 — 12 models (incl cap variants)")
    print("=" * 60)
    ...
    # v60 setup C (paradigm 추가)
    st60 = np.load(CACHE_DIR / "v60_state.npz")
    v60_oof = kalman_train + st60["oof_C"] * ALPHA
    v60_te  = kalman_test  + st60["test_C"] * ALPHA
    ...
    models_train = [v30A_oof, v30B_oof, v35_oof, v41A_oof, v41B_oof, v44_oof, gate_oof,
                    v39_oof, v32_oof, v52_05_oof, v52_15_oof, v60_oof]
    models_test  = [v30A_te,  v30B_te,  v35_te,  v41A_te,  v41B_te,  v44_te,  gate_te,
                    v39_te,  v32_te,  v52_05_te, v52_15_te, v60_te]
    ...
    oof_v61 = np.mean(oofs, axis=0)
    test_v61 = np.mean(tests, axis=0)
    w_mean = np.mean(all_w, axis=0)
    rh_v61 = rhit(oof_v61, y_train)

    print(f"\n=== v61 SoftStacker v6 결과 ===")
    print(f"  v48 (9m)      : 0.6734")
    print(f"  v53 (11m)     : 0.6746")
    print(f"  v61 (12m)     : {rh_v61:.4f}  (Δ vs v53: {rh_v61 - 0.6746:+.4f})")
    ...
    try:
        ...
        best_2 = (1.0, 0.0); best_2r = rh_v61
        for a in np.linspace(0, 1, 21):
            ens = a*oof_v61 + (1-a)*oof_v48_3way
            r = rhit(ens, y_train)
            if r > best_2r: best_2r, best_2 = r, (a, 1-a)
        print(f"\n  v59 × v48_3way: {best_2} OOF={best_2r:.4f}")

        best_3 = (1.0, 0.0, 0.0); best_3r = rh_v61
        for a in np.linspace(0, 1, 11):
            for b in np.linspace(0, 1-a, 11):
                c = 1-a-b
                ens = a*oof_v61 + b*oof_v53 + c*oof_v48_3way
                r = rhit(ens, y_train)
                if r > best_3r: best_3r, best_3 = r, (a, b, c)
        print(f"  3-way v59/v53/v48_3way: {best_3} OOF={best_3r:.4f}")
    except Exception as e:
        print(f"  hybrid cache err: {e}")
        best_2, best_2r = (1.0, 0.0), rh_v61
        best_3, best_3r = (1.0, 0.0, 0.0), rh_v61
        test_v48_3way = test_v53 = None

    np.savez(CACHE_DIR / "v61_state.npz",
             oof_v61=oof_v61, test_v61=test_v61, rh_v61=rh_v61,
             weights_oof_mean=w_mean)

    out_csv = DATA_DIR / "submission_v61_cpu.csv"
    pd.DataFrame({"id": sub["id"], "x": test_v61[:,0], "y": test_v61[:,1], "z": test_v61[:,2]}
                 ).to_csv(out_csv, index=False)
    print(f"\n  [submission] {out_csv}")

    if best_2r > rh_v61 and test_v48_3way is not None:
        a, b = best_2
        hyb = a*test_v61 + b*test_v48_3way
        hyb_csv = DATA_DIR / f"submission_v61_2way_v61x{a:.2f}_v48_3wayx{b:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb[:,0], "y": hyb[:,1], "z": hyb[:,2]}
                     ).to_csv(hyb_csv, index=False)
        print(f"  [2way] {hyb_csv.name}")

    if best_3r > rh_v61 and test_v53 is not None and test_v48_3way is not None:
        a, b, c = best_3
        hyb = a*test_v61 + b*test_v53 + c*test_v48_3way
        hyb_csv = DATA_DIR / f"submission_v61_3way_v61x{a:.2f}_v53x{b:.2f}_v48_3wayx{c:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb[:,0], "y": hyb[:,1], "z": hyb[:,2]}
                     ).to_csv(hyb_csv, index=False)
        print(f"  [3way] {hyb_csv.name}")

    entry = {
        "version": "v61_soft_stacker_v6",
        "ts": _dt.datetime.now().isoformat(timespec="seconds"),
        "approach": "12-model SoftStacker (v53 + v60 setup C paradigm)",
        "n_models": len(names),
        "model_names": names,
        "oracle_any12": float(oracle),
        "v61_oof": float(rh_v61),
        "delta_vs_v53": float(rh_v61 - 0.6746),
        "weight_avg": {n: float(wi) for n, wi in zip(names, w_mean.mean(axis=0))},
        "2way_v61_v48_3way": list(best_2),
        "2way_oof": float(best_2r),
        "3way_v61_v53_v48_3way": list(best_3),
    ...
    print(f"\n  [run_log] appended v61_soft_stacker_v6")
