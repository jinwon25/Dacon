"""v80_softstacker_v78.py — 14-model SoftStacker (v75 + v78).
...
def main():
    ...
    names = ["v30A","v30B","v35","v41A","v41B","v44","gate","v39","v32",
             "v73s","v73h","v65h","v62","v78"]
    K = len(names)
    print(f"v80 SoftStacker — {K} models")
    ...
    print(f"  v80 ({K}m): {rh_v:.4f}  (Δ vs base: {rh_v - rh_base:+.4f})")
    ...
    print(f"\n  hybrid base×w + v80: best w={best_w:.2f} → OOF {best_r:.4f}  Δ {best_r - rh_base:+.4f}")

    out = DATA_DIR / "submission_v80_14m.csv"
    ...
    if best_w < 1.0 and best_r > rh_base:
        ...
        h = DATA_DIR / f"submission_v80_hyb_basex{best_w:.2f}.csv"
        pd.DataFrame({"id": sub["id"], "x": hyb_t[:,0], "y": hyb_t[:,1], "z": hyb_t[:,2]}).to_csv(h, index=False)
        print(f"  [hybrid] OOF {best_r:.4f} → {h.name}")

    np.savez(CACHE_DIR / "v80_state.npz", oof=oof, test=test, rh=rh_v, w_mean=w_mean)
    entry = {"version": "v80_softstacker_v78", "ts": _dt.datetime.now().isoformat(timespec="seconds"),
