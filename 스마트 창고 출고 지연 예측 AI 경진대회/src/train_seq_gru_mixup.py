"""BiGRU with Manifold Mixup + SwapNoise auxiliary task (PyTorch CPU).

Verma et al. ICML 2019 Manifold Mixup + Yao et al. C-Mixup for regression.
- Input mixup with α=0.2 on raw features
- Manifold mixup with α=0.4 on hidden states (post-encoder)
- Target mix in log1p space
- SwapNoise denoising aux task (15% feature swap, 0.1× MSE recon)
- Smooth-L1 loss instead of L1 (heavy-tail robust)
"""
...
class SeqGRUMixup(nn.Module):
    def __init__(self, n_features: int, hidden: int = 64, n_layers: int = 2, dropout: float = 0.2):
        ...
        # SwapNoise aux: feature reconstruction
    ...
    def forward(self, x, mixup_lam=None, mixup_idx=None, manifold=False):
        ...
        if manifold and mixup_lam is not None and mixup_idx is not None:
            # manifold mixup post-encoder
            h = mixup_lam * h + (1 - mixup_lam) * h[mixup_idx]

        h = self.gru_forward(h)
...
def swap_noise(x, p=0.15):
    """random feature-value swap across batch (Jahrer 2017, Porto Seguro)."""
    B, T, F = x.shape
...
def main():
    ...
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "models/seqgru_mixup_seed42"))
    ...
    parser.add_argument("--use-smooth-l1", action="store_true", default=True)
    ...
    pred_loss_fn = nn.SmoothL1Loss(reduction="none", beta=1.0) if args.use_smooth_l1 else nn.L1Loss(reduction="none")
    ...
    for fold, (tr_idx, val_idx) in enumerate(make_folds(train, args.n_splits, args.seed), start=1):
        ...
        model = SeqGRUMixup(n_features=Xtr.shape[1], hidden=args.hidden,
        ...
        for epoch in range(args.epochs):
            ...
            for x, yt, mk in tr_loader:
                ...
                # reconstruction loss (target = original x at swap positions)
